...
do
    ...
    #	Check if Magento is Community Edition or Enterprise Edition
        ...
        patchFile="${v_Basedir}app/etc/applied.patches.list"
        if [ ! -f "$patchFile" ]
        then
            continue;
        fi
    ...
    # 	v_5344=`cat ${patchFile} | grep 'SUPEE-5344' > /dev/null`
    # 	v_Status="${?}"
    # 	if [[ x"${v_Status}" == x"0" ]];then
    ...
    # 	v_5994=`cat ${patchFile} | grep 'SUPEE-5994' > /dev/null`
    # 	v_Status="${?}"
    # 	if [[ x"${v_Status}" == x"0" ]];then
   ...
   #	Get domain name associated with username
    ...
    #	Print Results
        ...
        echo "${v_Username},${v_Version},${v_Edition}"
done
