...
#	* This script takes in a number of options and will allow you the ability
#	to automatically run a series of commands on the remote machine
#	* After script completes you will get back a status code based on the result of the
#	command that ran on the remote server
#	* Data will be stored in a CSV
#	* This is multi threaded and can go through a list of dozens of machines in a matter of moments
#
...
# Usage
        ...
        echo "		-sf SOURCE FILE: Source file to copy
        echo "		-df DESTINATION FILE: Destination to copy to
...
# No arguments
	...
	if [ ! -f "$SOURCEFILE" ];then
		echo "Sourcefile $SOURCEFILE not found."
	fi
...
#  # count a number of processes, then wait a bit.
   ...
   (
	...
	#       # TCPKeepAlive and ServerAlive for timeouts (Helps)
		scp -o TCPKeepAlive=no -o ConnectTimeout=10 -o PasswordAuthentication=no -o ServerAliveInterval=5 -o StrictHostKeyChecking=no -o BatchMode=yes "${SOURCEFILE}" root@${HOST}:"${DESTFILE}"   > "$v_ssh_result_tmp" 2>&1
