...
check_file_status() {
    ...
    then
        ...
        echo "ERROR: Please check to make sure the certificate for ${HOST}:443 is valid"
        RETCODE=1
        return
    fi
    ...
    then
        ...
        prints ${HOST} 443 "Expired" "${CERTDATE}" "${CERTDIFF}" "${CERTISSUER}" "${COMMONNAME}" "${SERIAL}"
        RETCODE=2
    ...
    then
        ...
        prints ${HOST} 443 "Expiring" "${CERTDATE}" "${CERTDIFF}" "${CERTISSUER}" "${COMMONNAME}" "${SERIAL}"
        RETCODE=1

    else
        prints ${HOST} 443 "Valid" "${CERTDATE}" "${CERTDIFF}" "${CERTISSUER}" "${COMMONNAME}" "${SERIAL}"
        RETCODE=0
    fi
}
